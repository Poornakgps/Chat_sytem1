...
let ws = new WebSocket(`ws://${window.location.host}/ws/seller`);
...
function handleStatusChange() {
    let selectedStatus = document.getElementById("orderStatusSelect").value;
    console.log("Selected Status:", selectedStatus);  // Debugging output
}
function updateStatus() {
    let selectedStatus = document.getElementById("orderStatusSelect").value;
    console.log("Updating Status to:", selectedStatus);  // Debugging output
...
async function respondToBargain(approved, amount) {
    if (role !== 'seller') return;
...
function displayBargainResponse(data) {
    const messageDiv = document.createElement('div');
    messageDiv.className = `bargain-request ${data.type === 'approved' ? 'bargain-approved' : 'bargain-rejected'}`;
        ...
        messageDiv.innerHTML = `
            ...
            ${data.qr_code ? `
                <div class="payment-section">
                    <img src="${data.qr_code}" alt="Payment QR Code" style="width:200px;height:200px;margin:10px auto;display:block;">
                    <p>Please scan the QR code to make the payment</p>
                    ${role === 'seller' ? `<button onclick="confirmPayment(${data.chat_id})">Confirm Payment</button>` : ''}
                </div>
            ` : ''}
        `;
    } else {
...
async function clearChat() {
    ...
    try {
        const response = await fetch('/seller/clear_chat', {
            method: 'POST'
        });
